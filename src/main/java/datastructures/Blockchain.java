...
//	}
        ...
        public void createNewBlock(Transaction trans) throws Exception {
            ...
            block.mine(getLatestBlockHash());
            chainBlock(block);
        }
        
        public boolean validateBlockchain(ArrayList chain) {
            if (chain.size() <= 1) {
                return true;
            }

            Block ancestor, child;
            Block firstBlock = (Block) chain.get(0);
          
            if (firstBlock.getPreviousHash().equals(GENESIS_HASH)) {
                return false;
            }

            for (int i = 0; i < chain.size() - 1; i++) {
                ancestor = (Block) chain.get(i);
                child = (Block) chain.get(i + 1);
        ...
        public void replaceChain(ArrayList newChain) {
            if(validateBlockchain(newChain) && newChain.size() > blockchain.size()) {
                System.out.println("Received new blockChain");
                blockchain = newChain;
            }
        }
